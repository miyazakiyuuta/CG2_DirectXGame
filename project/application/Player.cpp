...
void Player::Update()
{
    ...
    if (!suppressTongueShotThisFrame_ && moveState_ != MovementState::CeilingCrawling && moveState_ != MovementState::WallClinging && input_->IsTriggerMouse(0)) {
    ...
    if (input_->IsTriggerKey(DIK_B)) {
        ...
        if (TryUseBeam(beamDir)) {
            ...
            // ビーム攻撃状態に入ったら、舌の薙ぎ攻撃も同時に開始してシナジーを出す
            if (tongue_ && !tongue_->IsBusy()) {

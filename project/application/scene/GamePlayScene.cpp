...
void GamePlayScene::Update()
{
        ...
        if (player_) {
            ...
            for (const auto& o : stage_->GetStageData().objects) {
                ...
                if (std::fabs(playerBottom - topY) <= kVertEps && std::fabs(ppos.x - o.position.x) <= halfX + 0.5f && std::fabs(ppos.z - o.position.z) <= halfZ + 0.5f) {
                    ...
                    Logger::Log(
                        std::string("Platform carry apply id:") + std::to_string(o.id) + " delta:" + std::to_string(delta.x) + "," + std::to_string(delta.y) + "," + std::to_string(delta.z) + " apply:" + std::to_string(applyDelta.x) + "," + std::to_string(applyDelta.y) + "," + std::to_string(applyDelta.z) + " playerPos:" + std::to_string(ppos.x) + "," + std::to_string(ppos.y) + "," + std::to_string(ppos.z) + "\n");
                    applied = true;
                    break;
                }
            }
    ...
    if (!stageEditor_->IsEditMode()) {
       ...
       if (enemiesInitializedFromStage_) {

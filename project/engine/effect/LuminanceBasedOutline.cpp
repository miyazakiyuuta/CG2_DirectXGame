...
void LuminanceBasedOutline::Initialize(DirectXCommon* dxCommon, SrvManager* srvManager) {
    ...
    // デフォルト：黒い線。strength=6は資料の「差が小さすぎるので適当に6倍」に合わせた値
    paramData_->lineColor = { 0.0f, 0.0f, 0.0f };
    paramData_->strength = 6.0f;
    paramData_->debugView = 0;
}

void LuminanceBasedOutline::Draw(uint32_t srcSrvIndex) {
...
#ifdef USE_IMGUI
    ImGui::SliderFloat("Strength", &paramData_->strength, 0.0f, 20.0f);
    ImGui::ColorEdit3("Line Color", &paramData_->lineColor.x);
    bool debugView = paramData_->debugView != 0;
    if (ImGui::Checkbox("Debug View (edge)", &debugView)) {
        paramData_->debugView = debugView ? 1 : 0;
    }
#endif
}
...
void LuminanceBasedOutline::CreateGraphicsPipelineState() {
    CreateRootSignature();
    ...
    Microsoft::WRL::ComPtr<IDxcBlob> pixelShaderBlob =
        dxCommon_->CompileShader(L"resources/shaders/LuminanceBasedOutline.PS.hlsl", L"ps_6_0");
